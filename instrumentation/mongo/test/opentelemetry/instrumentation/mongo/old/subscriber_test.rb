...
require_relative '../../../../../lib/opentelemetry/instrumentation/mongo/subscribers/old/subscriber'

# Tests for old semantic convention attributes (db.system, db.name, db.operation, net.peer.name, net.peer.port)
describe OpenTelemetry::Instrumentation::Mongo::Subscribers::Old::Subscriber do
  ...
  before do
    skip unless ENV['BUNDLE_GEMFILE']&.include?('old') || ENV['BUNDLE_GEMFILE'].nil?

    # Ensure old semconv mode (default - no env var)
  ...
  describe 'db_statement include option' do
    ...
    it 'includes non-obfuscated db.statement attribute' do
      _(span.name).must_equal 'people.find'
      _(span.attributes['db.operation']).must_equal 'find'
      _(span.attributes['db.mongodb.collection']).must_equal 'people'
  ...
  describe 'a failed query' do
    ...
    describe 'that triggers #failed before #started' do
      let(:subscriber) { OpenTelemetry::Instrumentation::Mongo::Subscribers::Old::Subscriber.new }
