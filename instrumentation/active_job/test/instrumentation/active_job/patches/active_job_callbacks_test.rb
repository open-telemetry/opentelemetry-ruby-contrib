...
describe OpenTelemetry::Instrumentation::ActiveJob::Patches::ActiveJobCallbacks do
  ...
  describe 'attributes' do
    ...
    describe 'messaging.active_job.enqueued_at' do
      it "is set when processing inline jobs" do
        ...
        ActiveJob::Base.queue_adapter = :inline
        TestJob.perform_later
        _(send_span.attributes['messaging.active_job.enqueued_at']).must_be_nil
        assert(process_span.attributes['messaging.active_job.enqueued_at'])
      end
    ...
    describe 'messaging.active_job.enqueued_duration_ms' do
      it "is set when processing inline jobs" do
        begin
          ActiveJob::Base.queue_adapter.shutdown
        rescue StandardError
        ...
        ActiveJob::Base.queue_adapter = :inline
        TestJob.perform_later
        _(send_span.attributes['messaging.active_job.enqueued_duration_ms']).must_be_nil
        assert(process_span.attributes['messaging.active_job.enqueued_duration_ms'])
      end
