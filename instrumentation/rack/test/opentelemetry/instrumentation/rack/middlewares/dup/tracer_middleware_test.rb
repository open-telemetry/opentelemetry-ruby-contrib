...
require_relative '../../../../../../lib/opentelemetry/instrumentation/rack/middlewares/dup/tracer_middleware'

describe OpenTelemetry::Instrumentation::Rack::Middlewares::Dup::TracerMiddleware do
  ...
  let(:described_class) { OpenTelemetry::Instrumentation::Rack::Middlewares::Dup::TracerMiddleware }
    ...
    describe 'config[:untraced_endpoints]' do
      describe 'when an array is passed in' do
        ...
        it 'does not trace paths listed in the array' do
          ...
          ping_span_stable = finished_spans.find { |s| s.attributes['url.path'] == '/ping' }
          _(ping_span).must_be_nil
          _(ping_span_stable).must_be_nil
      ...
      describe 'when a string is passed in' do
        ...
        it 'traces everything' do
          ...
          _(ping_span_stable).wont_be_nil
    ...
    describe 'config[:untraced_requests]' do
      describe 'when a callable is passed in' do
        ...
        it 'does not trace requests in which the callable returns true' do
          ...
          ping_span_stable = finished_spans.find { |s| s.attributes['url.path'] == '/assets' }
          _(ping_span).must_be_nil
          _(ping_span_stable).must_be_nil
  ...
  describe 'config[:public_request]' do
    ...
    describe 'public_request with config on headers and path' do
      ...
      it 'creates a new root span on request to public path without custom header' do
        ...
        Rack::MockRequest.new(rack_builder).get('/public-path', req_env)
        _(first_span.parent_span_id).must_equal OpenTelemetry::Trace::INVALID_SPAN_ID
        _(first_span.links.size).must_equal 1
        _(first_span.links.first.span_context.span_id).must_equal remote_span_context.span_id
        _(first_span.links.first.span_context.trace_id).must_equal remote_span_context.trace_id
      ...
      it 'creates a new root span on request to public path when custom header value is unknown' do
        ...
        req_env = {
          'HTTP_TRACEPARENT' => remote_headers['traceparent'],
          'HTTP_TRACESTATE' => remote_headers['tracestate'],
          'HTTP_X_APP_CLIENT_ID' => 'unknown-id'
        }
        Rack::MockRequest.new(rack_builder).get('/public-path', req_env)
        _(first_span.parent_span_id).must_equal OpenTelemetry::Trace::INVALID_SPAN_ID
        _(first_span.links.size).must_equal 1
      ...
      it 'attaches a remote context on request to other path' do
        ...
        Rack::MockRequest.new(rack_builder).get('/other-path', req_env)
