...
The days is to be calculated based on the expected age of the major version when it becomes the lowest major version which is not end of life.
In the above example, we want until a major version has been available for 1460 days (4 years) which is calculated based on each major version of Postgres being supported for 5 years with a new major each year.
