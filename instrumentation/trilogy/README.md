...
| ------ | ------- | ----------- |
| `db_statement` | `:obfuscate` | Deprecated, use `include_dbquerytext` on stable/dup. Controls how SQL queries appear in spans. `:obfuscate` replaces literal values with `?`, `:include` records the raw SQL, `:omit` excludes the attribute entirely. |
| `include_dbquerytext` | false | Defines if the db.query.text attribute should be included. |
